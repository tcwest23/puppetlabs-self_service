...
- name: Classifier Configuration Data
  data_hash: classifier_data')
      ...
      it 'if S0040 conditions for false are met' do
        ...
        run_shell('mv /etc/systemd/system/puppet_system_processes-metrics.timer /etc/systemd/system/puppet_system_processes-metrics.bak')
        result = run_shell('facter -p self_service.S0040')
        expect(result.stdout).to match(%r{false})
        run_shell('mv /etc/systemd/system/puppet_system_processes-metrics.bak /etc/systemd/system/puppet_system_processes-metrics.timer')
